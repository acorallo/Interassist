...
--------------------------------------------------------
 ...
 BEGIN
  ...
  -- EGV 10Jun2017 Fin
   ...
   IF(P_SEARCH IS NOT NULL) THEN 
   NULL;
    -- EGV 25Nov2017 Inicio
    --strsql:= strsql || IASSIST_PKG.WHERECLAUSE(isWhere) || 'UPPER(AFILIADO_PKG.SPLIT_NOMBRE_APELLIDO(A.NOMBRE, A.APELLIDO)||A.IDAFILIADO||A.POLIZA||A.IDDOCUMENTO||A.PATENTE||B.NOMBRE||C.NOMBRE) LIKE UPPER(''%' || P_SEARCH || '%'')';
    strsql:= strsql || IASSIST_PKG.WHERECLAUSE(isWhere) || 'UPPER(A.PATENTE||AFILIADO_PKG.SPLIT_NOMBRE_APELLIDO(A.NOMBRE, A.APELLIDO)||A.IDAFILIADO||A.POLIZA||A.IDDOCUMENTO||A.PATENTE||B.NOMBRE||C.NOMBRE) LIKE UPPER(''%' || P_SEARCH || '%'')';
    -- EGV 25Nov2017 Fin
  END IF;
